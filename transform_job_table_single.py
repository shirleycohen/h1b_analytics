...
with beam.Pipeline('DirectRunner', options=opts) as p:
    ...
    job_query_results = p | 'Read from BigQuery Job' >> beam.io.Read(beam.io.BigQuerySource(query='SELECT * FROM h1b_split.Job_Temp order by employer_name limit 100'))
    emp_query_results = p | 'Read from BigQuery Employer' >> beam.io.Read(beam.io.BigQuerySource(query='SELECT employer_id, employer_name, employer_city FROM ' \
                                                                                                       'h1b_split.Employer order by employer_name limit 500'))
