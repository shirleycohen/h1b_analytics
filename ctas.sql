-- Dependencies: 
-- 1) load raw h1b files as tables applications_2015, applications_2016, applications_2017 into dataset h1b_raw
...
(SELECT DISTINCT employer_name, employer_address, employer_city, employer_state, 
 employer_postal_code, employer_country, employer_province, employer_phone, employer_phone_ext, h1b_dependent, willful_violator 
 FROM `cs327e-fa2018.h1b_raw.H1B_Applications_2017`
 WHERE employer_name IS NOT NULL AND employer_city IS NOT NULL
 UNION DISTINCT
 SELECT DISTINCT employer_name, employer_address, employer_city, employer_state, 
 employer_postal_code, employer_country, employer_province, employer_phone, employer_phone_ext, h1b_dependent, willful_violator 
...
(SELECT DISTINCT employer_name, employer_city, CAST(employment_start_date AS DATE) as employment_start_date, CAST(employment_end_date AS DATE) as employment_end_date, 
 ...
 FROM `cs327e-fa2018.h1b_raw.H1B_Applications_2017`
 WHERE employer_name IS NOT NULL AND employer_city IS NOT NULL AND employment_start_date IS NOT NULL AND employment_end_date IS NOT NULL AND job_title IS NOT NULL
 UNION DISTINCT
 SELECT DISTINCT employer_name, employer_city, CAST(employment_start_date AS DATE) as employment_start_date, CAST(employment_end_date AS DATE) as employment_end_date, 
  job_title, wage_rate_of_pay_from, wage_rate_of_pay_to, wage_unit_of_pay, 
...
SELECT case_number, case_status, CAST(case_submitted AS DATE) as case_submitted, CAST(decision_date AS DATE) as decision_date, visa_class, 
employer_name, employer_city, attorney_id
FROM `cs327e-fa2018.h1b_raw.H1B_Applications_*` a LEFT OUTER JOIN `cs327e-fa2018.h1b_split.Attorney` t ON a.agent_attorney_name = t.attorney_name
