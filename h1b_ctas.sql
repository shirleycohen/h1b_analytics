-- Dependencies: 
-- 1) load raw h1b files as tables applications_<year> where year = 2015 - 2018 into dataset h1b_staging
-- 2) create new dataset h1b_modeled
-- 3) run each create-table-as-select statement in this script to create and populate the new tables in h1b_modeled. 

-- Create Employer_Temp tables and assign each record a unique employer_id 
-- Table contains duplicate employer records
-- TO DO: remove duplicates records through Beam
CREATE TABLE h1b_modeled.Employer_Temp AS
...
(SELECT DISTINCT employer_name, employer_address, employer_city, employer_state, 
 ...
 willful_violator 
 FROM h1b_staging.H1B_Applications_2018
 ...
 FROM h1b_staging.H1B_Applications_2017
 WHERE employer_name IS NOT NULL AND employer_name != '1' AND employer_city IS NOT NULL
 UNION DISTINCT
 SELECT DISTINCT employer_name, employer_address, employer_city, employer_state, 
 employer_postal_code, employer_country, employer_province, employer_phone, h1b_dependent, willful_violator 
 FROM h1b_staging.H1B_Applications_2016
 WHERE employer_name IS NOT NULL AND employer_name != '1' AND employer_city IS NOT NULL
 UNION DISTINCT
 ...
 FROM h1b_staging.H1B_Applications_2015
 WHERE employer_name IS NOT NULL AND employer_name != '1' AND employer_city IS NOT NULL
)
...
CREATE TABLE h1b_modeled.Job_Temp AS
...
(
 ...
 worksite_city, worksite_county, worksite_state, worksite_postal_code, soc_code, soc_name, total_workers, 
 SAFE_CAST(full_time_position AS BOOL) as full_time_position, 
 SAFE_CAST(prevailing_wage as FLOAT64) as prevailing_wage, pw_unit_of_pay, pw_source, 
 CAST(pw_source_year AS STRING) as pw_source_year, pw_source_other
 FROM h1b_staging.H1B_Applications_2018
 WHERE employer_name IS NOT NULL AND employer_name != '1' AND employer_city IS NOT NULL 
 AND employment_start_date IS NOT NULL AND employment_end_date IS NOT NULL AND job_title IS NOT NULL
 UNION DISTINCT
 SELECT DISTINCT employer_name, employer_city, CAST(employment_start_date AS DATE) as employment_start_date, 
 ...
 SELECT DISTINCT employer_name, employer_city, CAST(employment_start_date AS DATE) as employment_start_date, 
  ...
  worksite_city, worksite_county, worksite_state, worksite_postal_code, soc_code, soc_name, total_workers, 
  SAFE_CAST(full_time_position AS BOOL) as full_time_position, 
  prevailing_wage, pw_unit_of_pay, pw_wage_source as pw_source, pw_wage_source_year as pw_source_year, 
  pw_wage_source_other as pw_source_other
 FROM h1b_staging.H1B_Applications_2016
 WHERE employer_name IS NOT NULL AND employer_name != '1' AND employer_city IS NOT NULL 
 AND employment_start_date IS NOT NULL AND employment_end_date IS NOT NULL AND job_title IS NOT NULL
 UNION DISTINCT
 SELECT DISTINCT employer_name, employer_city, CAST(employment_start_date AS DATE) as employment_start_date, 
  CAST(employment_end_date AS DATE) as employment_end_date, 
  job_title, SAFE_CAST(TRIM(SUBSTR(wage_rate_of_pay, 0, (STRPOS(wage_rate_of_pay, "-")-1))) AS FLOAT64) 
  ...
  wage_unit_of_pay, worksite_city, worksite_county, worksite_state, worksite_postal_code, soc_code, soc_name, 
...
CREATE TABLE h1b_modeled.Attorney AS
...
FROM h1b_staging.H1B_Applications_*
...
CREATE TABLE h1b_staging.Application_Temp AS
SELECT case_number, case_status, case_submitted, decision_date, visa_class, 
employer_name, employer_city, attorney_id
FROM h1b_staging.H1B_Applications_2018 a LEFT OUTER JOIN h1b_modeled.Attorney t ON a.agent_attorney_name = t.attorney_name
...
employer_name, employer_city, attorney_id
FROM h1b_staging.H1B_Applications_2017 a LEFT OUTER JOIN h1b_modeled.Attorney t ON a.agent_attorney_name = t.attorney_name
AND a.agent_attorney_city = t.attorney_city
WHERE employer_name IS NOT NULL AND employer_name != '1' AND employer_city IS NOT NULL
UNION ALL
SELECT case_number, case_status, SAFE_CAST(case_submitted AS DATE) as case_submitted, 
       SAFE_CAST(decision_date AS DATE) as decision_date, visa_class, 
       employer_name, employer_city, attorney_id
FROM h1b_staging.H1B_Applications_2016 a LEFT OUTER JOIN h1b_staging.Attorney t ON a.agent_attorney_name = t.attorney_name
AND a.agent_attorney_city = t.attorney_city
WHERE employer_name IS NOT NULL AND employer_name != '1' AND employer_city IS NOT NULL
UNION ALL
SELECT case_number, case_status, SAFE_CAST(case_submitted AS DATE) as case_submitted, 
       SAFE_CAST(decision_date AS DATE) as decision_date, visa_class,
       employer_name, employer_city, attorney_id
FROM h1b_staging.H1B_Applications_2015 a LEFT OUTER JOIN h1b_staging.Attorney t ON a.agent_attorney_name = t.attorney_name
AND a.agent_attorney_city = t.attorney_city
WHERE employer_name IS NOT NULL AND employer_name != '1' AND employer_city IS NOT NULL;
